defmodule TaskBunny.Partition.KeySet do
  @moduledoc """
  An opaque interface to storing keys and testing set member ship of
  keys. Like a bloom filter, but 100% probabilistic.
      iex> {:ok, pid} = TaskBunny.Partition.KeySet.start_link
      iex> TaskBunny.Partition.KeySet.add(pid, "bla")
      :ok
      iex> TaskBunny.Partition.KeySet.contains?(pid, "bla")
      true
      iex> TaskBunny.Partition.KeySet.contains?(pid, "beh")
      false
  Keys can also be removed:
      iex> {:ok, pid} = TaskBunny.Partition.KeySet.start_link
      iex> TaskBunny.Partition.KeySet.add(pid, "bla")
      iex> TaskBunny.Partition.KeySet.remove(pid, "bla")
      :ok
      iex> TaskBunny.Partition.KeySet.contains?(pid, "bla")
      false
  ...
  is started.
      iex> {:ok, pid} = TaskBunny.Partition.KeySet.start_link
      iex> TaskBunny.Partition.KeySet.add(pid, "existing")
      iex> {:ok, state} = TaskBunny.Partition.KeySet.get_state(pid)
      iex> {:ok, pid2} = TaskBunny.Partition.KeySet.start_link(state: state)
      iex> TaskBunny.Partition.KeySet.contains?(pid2, "existing")
      true
  Many keys can also be added at once:
      iex> {:ok, pid} = TaskBunny.Partition.KeySet.start_link
      iex> TaskBunny.Partition.KeySet.add_many(pid, ~w(a b c))
      :ok
      iex> TaskBunny.Partition.KeySet.contains?(pid, "a")
      true
      iex> TaskBunny.Partition.KeySet.contains?(pid, "b")
      true
      iex> TaskBunny.Partition.KeySet.contains?(pid, "c")
      true
  ...
  applied to the first keyset; the second is left untouched:
      iex> {:ok, a} = TaskBunny.Partition.KeySet.start_link
      iex> TaskBunny.Partition.KeySet.add_many(a, ~w(a1 a2 a3))
      iex> {:ok, b} = TaskBunny.Partition.KeySet.start_link
      iex> TaskBunny.Partition.KeySet.add_many(b, ~w(b1 b2))
      iex> TaskBunny.Partition.KeySet.union(a, b)
      :ok
      iex> TaskBunny.Partition.KeySet.contains?(a, "b1")
      true
      iex> TaskBunny.Partition.KeySet.contains?(b, "a1")
      false
      iex> TaskBunny.Partition.KeySet.difference(a, b)
      :ok
      iex> TaskBunny.Partition.KeySet.contains?(a, "b1")
      false
